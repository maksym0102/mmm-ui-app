const electron = require("electron");
const { dialog } = require("electron");
const path = require("path");
const fs = require("fs");
const configDir = (electron.app || electron.remote.app).getPath("userData");

const getFilePath = (projectName) => {
  const filePath = path.join(configDir, "projects", projectName, "model");
  if (fs.existsSync(filePath)) {
    console.log("folder exists");
  } else {
    console.log("folder doesn't exist");
    fs.mkdir(filePath, { recursive: true }, (e) => {
      console.log(e);
    });
  }
  return filePath;
}

const base64_encode = (file) => {
  return "data:model/obj;base64," + fs.readFileSync(file, 'base64');
}

module.exports.getModels = (projectName) => {
  const modelConfigPath = getFilePath(projectName);
  const files = fs.readdirSync(modelConfigPath);
  return files;
};

module.exports.openFilePicker = async (projectName) => {
  const configPath = getFilePath(projectName);
  let file, fileName;
  const response = await dialog.showOpenDialog({ properties: ["openFile"] });
  if (response.canceled) {
    file = "no file selected";
  } else {
    file = response.filePaths[0];
    fileName = path.basename(file);
    fs.copyFileSync(file, path.join(configPath, fileName));
  }
};
